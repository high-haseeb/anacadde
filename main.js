...
class Game {
    constructor() {
        ...
        //============= debug UI
        const gui  = new GUI();
        const targetFolder = gui.addFolder("Position");
        targetFolder.add(this.targetPosition, 'x', -10, 10).name("X position");
        targetFolder.add(this.targetPosition, 'y', -10, 10).name("X position");
        targetFolder.add(this.targetPosition, 'z', -10, 10).name("X position");
        targetFolder.open();
        //============= debug UI
    ...
    setFlashingLabel(label) {
        ...
        if (this.mobileCheck()) {
            ...
            if (label == "İletişim") {
                this.targetPosition = new THREE.Vector3(temp.x, 1, temp.z).sub({ x: 7.0, y: 0, z: 7.0 });
            } else if (label == "Ana Servis") {
                this.targetPosition = new THREE.Vector3(temp.x, 1, temp.z).sub({ x: 8.0, y: 0, z: 7.0 });
            } else {
                this.targetPosition = new THREE.Vector3(temp.x, 1, temp.z).sub({ x: 7.0, y: 0, z: 6.0 });
            }
        }
